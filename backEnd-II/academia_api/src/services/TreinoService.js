...
class TreinoService {
  ...
  async cadastrarTreino(dados) {
    const { nome_aluno, instrutor, grupo_muscular, frequencia_semanal, ativo } =
      dados;
    if (!nome_aluno || !instrutor || !grupo_muscular || !frequencia_semanal) {
      throw {
        status: 400,
        mensagem:
          "Aluno, instrutor, grupo muscular e frequência são obrigatórios",
      };
    }
    if (
      typeof frequencia_semanal !== "number" ||
      frequencia_semanal < 1 ||
      frequencia_semanal > 7
    ) {
      throw {
        status: 400,
        mensagem: "A frequência semanal deve ser entre 1 e 7 dias",
      };
    }
  ...
  async atualizarTreino(id, dados) {
    ...
    if (grupo_muscular !== undefined)
      atualizado.grupo_muscular = grupo_muscular.trim();
